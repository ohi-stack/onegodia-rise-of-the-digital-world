  ...
  return (
    <div className="min-h-screen bg-[#050608] text-slate-300 flex flex-col font-sans selection:bg-blue-600 selection:text-white">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {activeTab === 'missions' && <MissionsView mission={mission} setActiveTab={setActiveTab} />}

        {activeTab === 'inventory' && (
          <InventoryView progress={progress} setProgress={setProgress} />
        )}

        {activeTab === 'developers' && <DevelopersView setActiveTab={setActiveTab} />}

        {activeTab === 'community' && <CommunityView setActiveTab={setActiveTab} />}

        {activeTab === 'web-doc' && <WebDocView />}

        {activeTab === 'players' && <PlayersView setActiveTab={setActiveTab} />}
